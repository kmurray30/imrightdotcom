...
**Extra scrutiny for dashboard 05 specifically:** every metric in dashboards 01-04 is a Counter or Histogram, both verified above. Dashboard 05 (`utils/image-cache.js`'s cache size gauges) introduces this codebase's first `ObservableGauge` instruments (`imright_image_cache_search_cache_queries` and friends) — same dot-to-underscore rule, but whether Gauges pick up any suffix at all through this OTLP→Prometheus pipeline (Counters get `_total`; Gauges typically get none) hasn't been checked against a live instance the way everything else here has. Do the Metrics browser check above before trusting dashboard 05's panels, and rename in the JSON if `imright_image_cache_lookups_total`, the seven `imright_image_cache_*` gauges, or the two new histograms (`imright_image_cache_embedding_similarity`, `imright_image_cache_embedding_latency_ms`) don't show up exactly as named.

**Dashboard 05's `thresholdsStyle`/threshold-line panels (7, 13) are also new territory:** every other dashboard here uses `thresholds` only on `stat` panels (a well-verified pattern — see dashboard 01/02's stat panels). Drawing a threshold as a line on a `timeseries` panel (`custom.thresholdsStyle.mode: "line"`) is a standard, long-standing Grafana core option, but it hasn't actually been round-tripped through this instance the way everything else in this doc has — if the threshold line doesn't render, that's the field to double-check first.

**Dashboard 05's `EMBEDDING_SIMILARITY_THRESHOLD` (currently 0.28) is a placeholder, not a measured value** — chosen from general CLIP ViT-B/32 knowledge, not from this app's real queries (there was no way to run the real model in the environment this was built in — see `utils/embeddings.js`'s own header comment). Once real traffic exists, panel 13 (embedding similarity by rank) is exactly the panel to look at to retune it: if rank-1 scores cluster just under the line, the threshold is probably too strict; if misses (panel 15) stay high while rank-1 scores look reasonable, it's probably too lenient. The threshold is env-tunable (`IMAGE_CACHE_SIMILARITY_THRESHOLD`) — no redeploy-the-code needed, just update panel 13's threshold line to match if you change it.
...
Histograms (`_bucket{le,...}` / `_sum` / `_count`): `imright_pipeline_time_to_ready_ms`, `imright_llm_latency_ms{provider,model,pipeline_step}`, `imright_external_latency_ms{service,operation}`, `imright_interaction_cost_usd`, `imright_interaction_tokens_total`, `imright_interaction_llm_calls`, `imright_interaction_llm_retries`, `imright_interaction_external_calls`, `imright_interaction_duration_ms`, `imright_image_cache_embedding_similarity{rank}`, `imright_image_cache_embedding_latency_ms{kind}`.

Gauges (no suffix expected, but unverified — see the callout above): `imright_image_cache_search_cache_queries`, `imright_image_cache_search_cache_distinct_images`, `imright_image_cache_metadata_images`, `imright_image_cache_downloaded_images`, `imright_image_cache_downloaded_bytes`, `imright_image_cache_db_file_bytes`, `imright_image_cache_embedded_images`.
