...
2. `WIKIMEDIA_USERNAME` and `WIKIMEDIA_PASSWORD` set. That's it for auth — `utils/wikimediaAuth.js` logs in automatically with these on first use; there's no separate token-minting step required to get started. (`wiki_searcher/scripts/wikimedia-login.js` is optional, for later: run it once to mint a `WIKIMEDIA_REFRESH_TOKEN`, set that instead, and you can drop `WIKIMEDIA_PASSWORD` so it's not sitting in an env var long-term.)
3. The vector index actually populated — run `wiki_searcher/scripts/build-index-from-wikimedia.js` once (downloads the Wikimedia Enterprise Snapshot and embeds it chunk-by-chunk on its own; no separate build step, and it needs a working `DATABASE_URL` since embedding happens as each chunk comes down, not afterward). `wiki_searcher/scripts/build-index-from-ndjson.js` is only for the narrower case of embedding an NDJSON file you already have from elsewhere. Then `wiki_searcher/scripts/refresh-daily.js` (scheduled, keeps the index current — see the script header for what it does and why it's safe to run on a schedule with zero impact on live requests).
